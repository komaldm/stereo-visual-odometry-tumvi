...
def align_sim3(model, data):
    """Aligns Monocular VO to Ground Truth while solving for unknown scale (Sim3)."""
...
def main():
    ...
    base_dir = os.path.join(project_root, "Results_C")
    ...
    gt_file = os.path.join(project_root, "dataset", "dataset-corridor3_512_16", "mav0", "mocap0", "data.csv")
    ...
    render_professional_metrics_table(metrics_summary, "1_Metrics_Table1.png")
    ...
    if err_data is not None and len(err_data[err_data > 0]) > 0:
        ...
        plt.savefig("2_Reprojection_Error1.png", dpi=300, bbox_inches='tight')
        plt.close()
    ...
    plt.savefig("3_Ground_Truth_Trajectory1.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.subplots_adjust(right=0.75)
    plt.savefig("4_Estimated_vs_GT1.png", dpi=300, bbox_inches='tight')
    ...
    anim.save("4a_Rotating_Trajectory.gif", dpi=150, writer='pillow')
    plt.close(fig_comp)
    ...
    fig_plotly.write_html("4b_Interactive_Trajectory.html")
    ...
    plt.subplots_adjust(right=0.75)
    plt.savefig("5_Estimated_Only1.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig("6_TopDown_View.png", dpi=300, bbox_inches='tight')
    plt.close()

    # ---------------------------------------------------------
    # NEW Figure 7: KEYFRAME EVOLUTION
    # ---------------------------------------------------------
    render_keyframe_evolution(kf_file, "7_Keyframe_Evolution.png")
    ...
    plt.plot(aligned_vo[:, 0], aligned_vo[:, 1], 'b--', label='Monoo VO', linewidth=1.5)
    ...
    plt.plot(relative_time, aligned_vo[:, 2], 'b--', label='Mono VO Z')
    ...
    plt.tight_layout()
    output_png = os.path.join(results_dir, "mono_performance_dashboard.png")
