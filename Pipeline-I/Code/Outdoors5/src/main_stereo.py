...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(project_root, "dataset", "dataset-outdoors5_512_16")
    ...
    # This guarantees the pipeline strictly uses the optimized outdoor defaults
    # (Global Bucketing, max_depth=40.0, epipolar_err=1.0) defined inside the class.
    ...
    for i in range(dataset.length - 1):
        ...
        # Extract native World-to-Camera transformations safely
        ...
        if "FAIL" in status or "LOST" in status or "WAITING" in status or "REJECTED" in status:
            ...
            if "KEYFRAME" in status or "METRIC_STABILIZED" in status or "TRACKING_OK" in status:
        ...
        # CRITICAL ALIGNMENT: Subtract the initial starting coordinates to anchor
        # the mapping natively to the Ground Truth start origin (0, 0, 0)
        ...
        # Unconditional seed logging guarantees accurate density statistics for every frame
        with open(seeds_file, "a") as f:
            f.write(f"{ts:.9f} {total_points}\n")

        # Unconditional keyframe status logging for the evaluation graph
